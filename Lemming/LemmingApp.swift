    ...
    var body: some Scene {
        WindowGroup {
            RootView(store: Store(initialState: RootFeature.State(postsRoot: .init(postsFeature: .init(postsList: .init(posts: [],
                                                                  ...
                                                                  account: .init(currentAccount: accountService.getCurrentAccount(), availableAccounts: accountService.getAccounts(), userProfile: .init(username: accountService.getCurrentAccount()?.id, items: [])),
